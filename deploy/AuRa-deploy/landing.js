...
/** Скролл к #features без появления index.html#features в адресе; старые ссылки с #features — скролл и чистый URL. */
function initFeaturesNavNoHash() {
  const toFeatures = (e) => {
    e.preventDefault();
  ...
  document.querySelectorAll('a[href="#features"]').forEach((a) => {
    a.addEventListener("click", toFeatures);
  });
...
initFeaturesNavNoHash();

/** Старые закладки /#cta — блок контактов теперь #contacts. */
...
const I18N = {
  ru: {
    ...
    "hero.ctaPrimary": "Скачать AuRa.APK (сборка __APK_BUILD__)",
  ...
  en: {
    ...
    "hero.ctaPrimary": "Download AuRa.APK (build __APK_BUILD__)",
